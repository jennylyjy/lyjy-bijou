...
function AdminPage() {
  ...
  const validateCashSale = async () => {
    ...
    try {
      ...
      const sale = { ticketNumber: `CAISSE-${Date.now().toString().slice(-6)}`, items: cashCart, total: cashTotal, loyaltyPointsEarned: earnedPoints, customerUid: cashCustomer?.pending ? null : cashCustomer?.id || null, customerName: cashCustomer ? `${cashCustomer.firstName || ""} ${cashCustomer.lastName || ""}`.trim() : "Client comptoir", customerEmail: cashCustomer?.email || "", paymentMethod: cashPaymentMethod, amountReceived: cashPaymentMethod === "Espèces" ? Number(cashAmountReceived) || cashTotal : cashTotal, change: cashPaymentMethod === "Espèces" ? cashChange : 0, status: "validated", source: "caisse", sessionId: cashSession.openedAt, createdAt: new Date().toISOString() };
      await addDoc(collection(db, "orders"), sale);
      if (cashCustomer?.id && !cashCustomer.pending) await updateDoc(doc(db, "users", cashCustomer.id), { loyaltyPoints: increment(earnedPoints) });
      if (cashCustomer?.pending) { const token = cashCustomer.id.split(":")[1]; await updateDoc(doc(db, "pendingCustomers", token), { loyaltyPoints: increment(earnedPoints) }); }
      setLastCashSale(sale);
      setCashCart([]); setCashAmountReceived(""); setCashPaymentOpen(false); setCashCustomer(null); setCashCustomerSearch(""); setSuccessMessage(`Vente enregistrée. ${earnedPoints} point${earnedPoints > 1 ? "s" : ""} fidélité gagné${earnedPoints > 1 ? "s" : ""}.`); setTimeout(() => setSuccessMessage(""), 3000);
