...
function AdminPage() {
  ...
  const addCashItem = (article: any, variant: any = null) => {
    const key = `${article.id}-${variant?.label || "article"}`;
      ...
      return [...current, { key, articleId: article.id, title: article.title, ref: variant?.ref || article.ref || "", category: article.category || "", price: Number(article.finalPrice ?? article.price) || 0, quantity: 1, variantLabel: variant?.label || "", variantSize: variant?.size || "" }];
    });
  };
  const handleCashBarcodeScan = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key !== "Enter") return;
    event.preventDefault();
    const normalizeScanCode = (value: unknown) => String(value || "").normalize("NFKC").replace(/[\u0000-\u001F\u007F]/g, "").trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
