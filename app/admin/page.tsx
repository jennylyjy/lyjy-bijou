...
function AdminPage() {
  ...
  const downloadReferencesPdf = () => {
    const rows = codeItems.map((item: any) => `<tr><td><img class="thumb" src="${item.imageUrl || "/logo.png"}" onerror="this.src='/logo.png'" /></td><td><strong>${item.title}</strong><br/><span class="muted">${item.label}</span></td><td class="ref">${item.ref}</td><td class="qty">${item.quantity}</td></tr>`).join("");
    const report = `<html><head><title>Références et stocks LYJY</title><style>body{font-family:Arial;padding:30px;color:#222}h1{color:#9a773f}p{color:#666}table{width:100%;border-collapse:collapse;margin-top:24px}th,td{border:1px solid #bbb;padding:9px;text-align:left;vertical-align:middle}th{background:#eee}.thumb{width:110px;height:110px;object-fit:cover;border:1px solid #ccc}.ref{font-family:monospace;font-size:15px}.qty{font-size:18px;font-weight:bold}.muted{color:#777;font-size:12px}</style></head><body><h1>LYJY Atelier — Références et stocks</h1><p>Chaque ligne indique la photo, l’article, la référence et la quantité disponible.<br/>Généré le ${new Date().toLocaleString("fr-FR")}</p><table><thead><tr><th>Photo</th><th>Article</th><th>Référence</th><th>Quantité</th></tr></thead><tbody>${rows || `<tr><td colspan="4">Aucune référence</td></tr>`}</tbody></table></body></html>`;
