...
function AdminPage() {
  ...
  const validateCashSale = async () => {
    ...
    try {
      ...
      const sale = { ticketNumber: `CAISSE-${Date.now().toString().slice(-6)}`, items: cashCart, total: cashTotal, paymentMethod: cashPaymentMethod, amountReceived: cashPaymentMethod === "Espèces" ? Number(cashAmountReceived) || cashTotal : cashTotal, change: cashPaymentMethod === "Espèces" ? cashChange : 0, status: "validated", source: "caisse", sessionId: cashSession.openedAt, createdAt: new Date().toISOString() };
