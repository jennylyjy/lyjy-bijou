...
export async function POST(request: Request) {
  try {
    ...
    await new Resend(key).emails.send({
      ...
      html: `<div style="font-family:Arial,sans-serif;color:#eee;max-width:620px;margin:auto;background:#171313"><div style="background:#0b0909;padding:28px;text-align:center"><img src="${origin}/logo.png" alt="LYJY" style="width:130px;background:#fff;padding:8px" /></div><div style="padding:34px"><p style="font-family:Georgia,serif;font-size:20px;color:#c4a77d">Bonjour ${firstName || ""},</p><p style="line-height:1.7">Merci pour votre achat chez LYJY Atelier. Votre compte client vous permettra de retrouver vos points fidélité et vos commandes.</p><p style="line-height:1.7">Cliquez sur le bouton ci-dessous pour choisir votre mot de passe et activer votre compte.</p><p style="text-align:center;margin:32px 0"><a href="${link.replaceAll("&", "&amp;")}" style="background:#c4a77d;color:#111;padding:15px 26px;text-decoration:none;display:inline-block">Créer mon mot de passe</a></p><p style="font-size:13px;line-height:1.6">Si le bouton ne fonctionne pas, copiez ce lien dans votre navigateur :<br><span style="word-break:break-all">${link}</span></p><hr style="border:0;border-top:1px solid #4b4038"><p style="font-size:12px;color:#aaa;text-align:center">© ${year} LYJY Atelier · www.lyjy.fr</p></div></div>`,
