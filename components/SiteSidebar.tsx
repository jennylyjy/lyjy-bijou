...
import { ShoppingBag, UserRound, LogIn, UserPlus, ChevronDown, RotateCcw } from "lucide-react";
...
export default function SiteSidebar() {
  ...
  const [openFilter, setOpenFilter] = useState<TaxonomyKey | null>(null);
  ...
  return <aside className="fixed left-0 top-0 bottom-0 z-40 hidden w-64 overflow-y-auto border-r border-stone-800 bg-black px-5 py-7 text-stone-300 lg:block">
    <Link href="/boutique" className="mb-8 block font-serif text-lg tracking-[0.2em] text-[#C4A77D]">LYJY</Link>
    ...
    <div className="space-y-2 py-5">
      ...
      {filterKeys.map((key) => <div key={key} className="border-b border-stone-900">
        <button type="button" onClick={() => setOpenFilter(openFilter === key ? null : key)} className="flex w-full items-center justify-between px-3 py-3 text-left text-xs uppercase tracking-widest hover:text-[#C4A77D]">{taxonomyLabels[key]}<ChevronDown className={`h-4 w-4 transition-transform ${openFilter === key ? "rotate-180" : ""}`} /></button>
        {openFilter === key && <div className="space-y-1 px-3 pb-3"><button type="button" onClick={() => setFilter(key, "all")} className="block w-full py-1 text-left text-xs text-[#C4A77D]">Tous</button>{taxonomy[key].filter(item => item.isVisible).map(item => <button type="button" key={item.id} onClick={() => setFilter(key, item.id)} className={`block w-full py-1 text-left text-xs hover:text-[#C4A77D] ${searchParams.get(key) === item.id ? "text-[#C4A77D]" : "text-stone-400"}`}>{item.label}</button>)}</div>}
      </div>)}
    </div>
